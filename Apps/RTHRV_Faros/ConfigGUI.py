...
def ConfigGUI(title="Config",
    ...
    *A TRICK:
        ...
        if type(configDict[entry]) is tuple or \
                type(configDict[entry]) is list:  # If description given:
                    ...
                    try:
                        ...
                        # It's a list or tuple but there's no string...
                        configDict[entry] = (configDict[entry][0], "")
    ...
    def castString(valuesDict, varName):
        ...
        # First if the user put in their own quotes,
        # remove them (assuming symmetric):
        try:
            while str(valuesDict[varName])[0] == "\"":
                valuesDict[varName] = str(valuesDict[varName])[1:-1]
        except IndexError:
            pass

        # Then reinterpret boolean strings:
        if valuesDict[varName] == "True":
            ...
            try:
                ...
                try:
                    ...
                    # Must be tuple, list, or string, but all of
                    # those will work with conversions to any of the
                    # others... So, a different kind of test:
                    if str(valuesDict[varName])[0] == "(":
                        return literal_eval(str(valuesDict[varName]))
                    elif str(valuesDict[varName])[0] == "[":
                        return literal_eval(str(valuesDict[varName]))
                    else:
                        return str(valuesDict[varName])
    ...
    def append(name, var, writeString):
        ...
        if type(var) is str:  # needs quotation marks
            ...
            if not((var[0] == "\"") and (var[-1] == "\"")):
                ...
                # No new quotation marks necessary:
                writeString = writeString + name + " = " + str(var) + "\n"
        else:
            # Assume float, int, tuple, or list.
            # Therefore needs no quotation marks:
            writeString = writeString + name + " = " + str(var) + "\n"
        return writeString
    ...
    ########################################################################"""
    import sys
    sys.path.insert(1, os.path.join("..","Apps","RTHRV_Faros"))
    
    #if os.path.exists("Config.py"):
    if os.path.exists(os.path.join("..","Apps","RTHRV_Faros","Config.py")):
    ...
    ConfigFile = open(os.path.join("..","Apps","RTHRV_Faros",
                                   "Config.py"), "w")  # open to overwrite
